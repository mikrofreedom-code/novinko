...
import { keywordReFor } from '../sections/index.js';
...
// --- Layer B/C (feed, text): čerstvosť + (pri cross-topic feedoch) keyword filter sekcie ---
function gateFeed(item) {
  ...
  // Cross-topic feedy (regulátori pri krypte, NVIDIA/Research pri AI): pusti len tému sekcie.
  const keywordFilter = rd.keyword_filter ?? rd.crypto_filter; // spätná kompat so starými raw
  if (keywordFilter) {
    const sectionId = rd.section ?? 'krypto';
    if (!keywordReFor(sectionId).test(`${rd.title} ${rd.text ?? ''}`)) {
      return { ok: false, reason: `nie téma sekcie '${sectionId}'` };
    }
  }
  return { ok: true };
}
