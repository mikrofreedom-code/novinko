...
// Základné skóre podľa typu udalosti. Ladené podľa redakčnej preferencie:
// širšie správy (oznámenia, partnerstvá, regulácie, listingy) HORE,
// technické governance DOLE (väčšina spadne pod latku 50).
const EVENT_BASE = {
  regulatory: 90,        // MiCA, SEC, centrálne banky — najdôležitejšie
  announcement: 72,      // oznámenia, partnerstvá, PR — to, čo chceme
  listing: 68,           // nové listingy = správa
  protocol_release: 55,  // upgrade protokolu (čitateľný ľudský štýl)
...
export function scoreImportance(facts) {
  ...
  if ((facts.facts ?? []).some((f) => /governance/i.test(f.source_name || ''))) {
    return { score: clamp(EVENT_BASE.governance + confluence, 0, 100),
