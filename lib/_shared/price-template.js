...
// Rovnaké pásma ako alternative.me Fear & Greed Index. Klasifikácia sa POČÍTA
// z hodnoty, nespolieha na `facts.fng_classification` — 06-chief-editor
// mergeClusterFacts() toto pole pri zlúčení zahodí (kopíruje len vybraný
// zoznam kľúčov), takže by tu bolo nespoľahlivo prázdne.
function classifyFearGreed(value) {
  if (value <= 24) return 'extrémny strach';
  if (value <= 44) return 'strach';
  if (value <= 55) return 'neutrál';
  if (value <= 75) return 'chamtivosť';
  return 'extrémna chamtivosť';
}
...
export function trySentimentTemplate(facts) {
  ...
  const sk = classifyFearGreed(value);
  const high = value >= 50;
