...
async function factsFromText(item, meta) {
  ...
  return { entity: parsed.entity ?? meta.entity ?? null, event_type, facts };
}

// ---------- Spoluj fakty do finálneho facts JSON ----------
function buildFacts({ entity, event_type, facts }) {
...
export async function run(item) {
  ...
  // Ak sú aj metriky aj text, uprednostni textom určený event_type ak nie je 'other'.
  if (event_type === 'other' && rd.metrics) event_type = 'price_move';

  const facts = buildFacts({ entity, event_type, facts: collected });
