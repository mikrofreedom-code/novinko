...
// Feedy re-listujú tie isté položky (rovnaký guid) celé dni, kým scout beží
// každých 30 min → bez kontroly by sa tá istá správa vkladala znova pri
// KAŽDOM behu (02-gateway ju síce neskôr odfiltruje, ale zaplaví to 'raw'
// rýchlejšie, než to gateway stíha spracovať). Preto pre feedy: never
// re-insert an already-known dedup_key, bez ohľadu na jeho aktuálny status.
//
// POZOR: toto sa NESMIE použiť pre CoinGecko/DefiLlama — ich dedup_key
// (`cg:<id>`, `dl:<id>`) je zámerne stabilný naprieč behmi, lebo gateway
// potrebuje VIDIEŤ každý nový cenový snímok (a duplicity rieši sám cez
// časové okno DEDUP_WINDOW_H). Blanket dedup by cenové sledovanie umlčal
// natrvalo po prvom behu.
async function insertFeedRows(rows) {
  if (rows.length === 0) return 0;
  const keys = [...new Set(rows.map((r) => r.raw_data?.dedup_key).filter(Boolean))];
  let seen = new Set();
  if (keys.length > 0) {
    const { data, error } = await db.from('queue')
      .select('raw_data->>dedup_key')
      .in('raw_data->>dedup_key', keys);
    if (error) throw error;
    seen = new Set((data ?? []).map((r) => r.dedup_key));
  }
  return insertRows(rows.filter((r) => !seen.has(r.raw_data?.dedup_key)));
}
...
async function scoutFeeds() {
  ...
  for (const feed of FEEDS) {
    try {
      ...
      total += await insertFeedRows(rows);
    } catch (err) {
