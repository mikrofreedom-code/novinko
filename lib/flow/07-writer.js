...
- Radšej kratší, jasný článok než dlhý nabitý údajmi.
...
// Koľko článkov maximálne napísať za jeden beh (= za hodinu pri hodinovom crone).
...
export async function runBatch(limit = 50) {
  ...
  // ŠKRT: napíš len top N najdôležitejších; zvyšok nechaj čakať v 'clustered'.
  const top = [...items]
    .sort((a, b) => (b.facts?.importance ?? 0) - (a.facts?.importance ?? 0))
    .slice(0, MAX_PER_RUN);

  const results = { ok: 0, failed: 0, skipped: Math.max(items.length - top.length, 0) };
